...
class GurobiVisitor(visitor.FormVisitor) :
    
    def __init__(self, model) :
        self.model = model # the scip model
        self.vars = {} # a dict to save the vars of the model
        self.number_aux_vars = 0
        self.prop_vars = [] # a list of propositional vars
 
    def visit_var(self, var) :
        if  not (str(var) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(var))
            self.vars[str(var)] = newVar
            self.prop_vars.append(str(var))
    
    def visit_constant(self, cons) :
        if  not (str(cons) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(cons))
            self.vars[str(cons)] = newVar
            self.model.addConstr(newVar == eval(str(cons)))
    ...
    def visit_max(self, max) :
        ...
        if not (str(max) in self.vars) :
            newVar =  self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(max))   # the new var for the expression
            self.vars[str(max)] = newVar # the var for the current formula is added
            auxVarName = self.new_aux_var()
            auxVar = self.model.addVar(vtype=GRB.BINARY,name=auxVarName)
            self.model.addConstr(auxVar >= self.vars[str(max.left)] - self.vars[str(max.right)])
            self.model.addConstr(1 - auxVar >= self.vars[str(max.right)] - self.vars[str(max.left)])
            self.model.addConstr(newVar == auxVar * self.vars[str(max.left)] + (1-auxVar)* self.vars[str(max.right)])
    ...
    def visit_min(self, min) :
        ...
        if not (str(min) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(min)) # the new var for the expression
            self.vars[str(min)] = newVar # the var for the current formula is added
            auxVarName = self.new_aux_var()
            auxVar = self.model.addVar(vtype=GRB.BINARY,name=auxVarName)
            self.model.addConstr(auxVar >= self.vars[str(min.left)] - self.vars[str(min.right)])
            self.model.addConstr(1 - auxVar >= self.vars[str(min.right)] - self.vars[str(min.left)])
            self.model.addConstr(newVar == auxVar * self.vars[str(min.right)] + (1-auxVar)* self.vars[str(min.left)])
    ...
    def visit_pand(self, pand) :
        ...
        if not (str(pand) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(pand))
            self.vars[str(pand)] = newVar
            self.model.addConstr(newVar == self.vars[str(pand.left)] * self.vars[str(pand.right)])
    ...
    def visit_por(self, por) :
        ...
        if not (str(por) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(por))
            self.vars[str(por)] = newVar
            self.model.addConstr(newVar == (self.vars[str(por.left)] + self.vars[str(por.right)]) - (self.vars[str(por.left)] * self.vars[str(por.right)]))

    def visit_pnot(self, pnot) :
        # we assume that the vars for the left and right are already in the model if not an exception is raised
        assert str(pnot.operand) in self.vars, "the operand is not in the model"
        if not (str(pnot) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.BINARY,name=str(pnot)) # the new var for the expression
            self.model.setObjective(newVar, GRB.MINIMIZE)
            self.vars[str(pnot)] = newVar # the var for the current formula is added
            self.model.addConstr(newVar >= (1 - self.vars[str(pnot.operand)]))
            #self.model.addConstr(newVar + self.vars[str(pnot.operand)]  <= 1.99999)  
    ...
    def visit_pimplies(self,  pimplies) :
        ...
        if not (str(pimplies) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(pimplies)) # the new var for the expression
            self.vars[str(pimplies)] = newVar # the var for the current formula is added
            auxVarName = self.new_aux_var()
            auxVarName2 = self.new_aux_var()
            auxVar = self.model.addVar(vtype=GRB.BINARY,name=auxVarName)
            self.model.setObjective(auxVar, GRB.MINIMIZE)
            auxVar2 = self.model.addVar(vtype=GRB.CONTINUOUS,lb = 0, ub = 1,name=auxVarName2)
            self.model.addConstr(auxVar >= self.vars[str(pimplies.right)] - self.vars[str(pimplies.left)])
            self.model.addConstr(1-auxVar >= self.vars[str(pimplies.left)] - self.vars[str(pimplies.right)])
            self.model.addConstr(newVar  == (auxVar2 * (1-auxVar)) +  (auxVar))
            #self.model.addConstr(auxVar2*self.vars[str(pimplies.left)] == self.vars[str(pimplies.right)])
            self.model.addConstr(newVar  == ((self.vars[str(pimplies.right)]/self.vars[str(pimplies.left)]) * (1-auxVar)) + auxVar) # check issues with division by zero
    ...
    def visit_land(self, land) :
        ...
        if not (str(land) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(land))
            self.vars[str(land)] = newVar # the var for the current formula is added
            auxVarName = self.new_aux_var()
            auxVar = self.model.addVar(vtype=GRB.BINARY,name=auxVarName)
            self.model.addConstr(auxVar >= 1 - self.vars[str(land.left)] - self.vars[str(land.right)])
            self.model.addConstr((1-auxVar) >= self.vars[str(land.left)] + self.vars[str(land.right)] - 1)
            self.model.addConstr(newVar ==  (1-auxVar)*(self.vars[str(land.left)] + self.vars[str(land.right)] - 1))
    ...
    def visit_limplies(self, limplies) :
        ...
        if not (str(limplies) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(limplies))
            self.vars[str(limplies)] = newVar # the var for the current formula is added
            auxVarName = self.new_aux_var()
            auxVar = self.model.addVar(vtype=GRB.BINARY,name=auxVarName)
            self.model.addConstr(auxVar >= self.vars[str(limplies.left)] - self.vars[str(limplies.right)])
            self.model.addConstr((1-auxVar) >= self.vars[str(limplies.right)] - self.vars[str(limplies.left)])
            self.model.addConstr(newVar ==  auxVar + (1- auxVar)*(1 - self.vars[str(limplies.left)] + self.vars[str(limplies.right)]))
    ...
    def visit_lor(self, lor) : 
        ...
        if not (str(lor) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(lor))
            self.vars[str(lor)] = newVar # the var for the current formula is added
            auxVarName = self.new_aux_var()
            auxVar = self.model.addVar(vtype=GRB.BINARY,name=auxVarName)
            self.model.addConstr(auxVar >= 1 - self.vars[str(lor.left)] - self.vars[str(lor.right)])
            self.model.addConstr((1-auxVar) >= self.vars[str(lor.right)] + self.vars[str(lor.left)] - 1)
            self.model.addConstr(newVar == auxVar + (1- auxVar)*(self.vars[str(lor.left)] + self.vars[str(lor.right)]))
    ...
    def visit_lnot(self, lnot) : 
        ...
        if not (str(lnot) in self.vars) :
            newVar = self.model.addVar(vtype=GRB.CONTINUOUS, lb = 0, ub = 1,name=str(lnot))
            self.vars[str(lnot)] = newVar # the var for the current formula is added
            self.model.addConstr(newVar ==  1 - self.vars[str(lnot.operand)])
            #auxVarName = self.new_aux_var()
